(() => {
  ...
  var searchClient = algoliasearch(api_id, api_key);
  ...
  var { poweredBy } = instantsearch.widgets;
  search.addWidgets([
    configure({
      // hitsPerPage: 5,
      ...
      templates: {
        item(hit, { html, components }) {
          ...
          <a href="${hit.permalink}" class="flex flex-col justify-around space-y-2 hover:text-white">
            <p class="text-lg">${components.Snippet({ hit, attribute: snippet })}</p>
            <p class="text-sm">${components.Highlight({ hit, attribute: highlight })}</p>
          </a>
        `;
        },
        empty(results, { html }) {
