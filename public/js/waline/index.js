...
\u76EE\u524D\u5B57\u6578\uFF1A$2`, "\u533F\u540D", "\u6F5B\u6C34", "\u5192\u6CE1", "\u5410\u69FD", "\u6D3B\u8E8D", "\u591A\u8A71", "\u50B3\u8AAA", "\u8868\u60C5\u5305", "\u641C\u5C0B\u8868\u60C5\u5305", "\u500B\u4EBA\u8CC7\u6599", "\u901A\u904E", "\u5F85\u5BE9\u6838", "\u5783\u573E", "\u53D6\u6D88\u7F6E\u9802", "\u6700\u65E9", "\u6700\u65B0", "\u71B1\u9580", "\u4F60\u8A8D\u70BA\u9019\u7BC7\u6587\u7AE0\u600E\u9EBC\u6A23\uFF1F"]);
  ...
  var Rn = (n2) => [{ name: "blockMath", level: "block", tokenizer(o2) {
    const t2 = En.exec(o2);
    if (t2 !== null)
      return { type: "html", raw: t2[0], text: n2(true, t2[1]) };
    ...
    const t2 = Ln.exec(o2);
    if (t2 !== null)
      return { type: "html", raw: t2[0], text: n2(false, t2[1]) };
  } }];
  ...
  var Qn = defineComponent({ __name: "ArticleReaction", setup(n2, { expose: o2 }) {
    o2();
    const t2 = Zn(), e2 = inject("config"), i = ref(-1), s2 = ref([]), r2 = computed2(() => e2.value.locale), u2 = computed2(() => e2.value.reaction.length > 0), l = computed2(() => {
  ...
  var ho = defineComponent({ __name: "CommentBox", props: { edit: { default: null }, rootId: { default: "" }, replyId: { default: "" }, replyUser: { default: "" } }, emits: ["log", "cancelEdit", "cancelReply", "submit"], setup(n2, { expose: o2, emit: t2 }) {
    o2();
    const e2 = n2, i = t2, s2 = inject("config"), r2 = Gn(), u2 = On(), l = _e(), a = ref({}), g = ref(null), b = ref(null), A = ref(null), f2 = ref(null), S = ref(null), E = ref(null), _ = ref(null), L = ref({ tabs: [], map: {} }), M = ref(0), x = ref(false), G = ref(false), se = ref(false), ce = ref(""), oe = ref(0), H = reactive({ loading: true, list: [] }), ne = ref(0), X = ref(false), ue = ref(""), Y = ref(false), le = ref(false), J = computed2(() => s2.value.locale), y2 = computed2(() => {
      var v2;
      return !!((v2 = l.value) != null && v2.token);
    }), R2 = computed2(() => s2.value.imageUploader !== false), N = (v2) => {
...
![${v2.name}](${w2})`);
      ...
      if (!e2.edit)
        if ((v2 = l.value) != null && v2.token)
          U2.nick = l.value.display_name, U2.mail = l.value.email, U2.link = l.value.url;
        else {
          if (ge === "force")
            return;
          if (Pe.indexOf("nick") > -1 && !U2.nick)
            return (h2 = a.value.nick) == null || h2.focus(), alert(J.value.nickError);
          if (Pe.indexOf("mail") > -1 && !U2.mail || U2.mail && !bn(U2.mail))
            return (w2 = a.value.mail) == null || w2.focus(), alert(J.value.mailError);
      ...
      if (!X.value)
        return alert(J.value.wordHint.replace("$0", He[0].toString()).replace("$1", He[1].toString()).replace("$2", oe.value.toString()));
      ...
      try {
        ...
        if (Y.value = false, Le.errmsg)
          return alert(Le.errmsg);
  ...
  var $o = defineComponent({ __name: "CommentCard", props: { comment: {}, edit: { default: null }, rootId: {}, reply: { default: null } }, emits: ["log", "submit", "delete", "edit", "like", "status", "sticky", "reply"], setup(n2, { expose: o2, emit: t2 }) {
    o2();
    const e2 = n2, i = t2, s2 = ["approved", "waiting", "spam"], r2 = inject("config"), u2 = ft(), l = useNow(), a = _e(), g = computed2(() => r2.value.locale), b = computed2(() => {
      const { link: x } = e2.comment;
      return x ? ct(x) ? x : `https://${x}` : "";
    }), A = computed2(() => u2.value.includes(e2.comment.objectId)), f2 = computed2(() => wn(new Date(e2.comment.time), l.value, g.value)), S = computed2(() => a.value.type === "administrator"), E = computed2(() => e2.comment.user_id && a.value.objectId === e2.comment.user_id), _ = computed2(() => {
      var x;
  ...
  var Sl = defineComponent({ __name: "WalineComment", props: ["serverURL", "path", "meta", "requiredMeta", "dark", "commentSorting", "lang", "locale", "pageSize", "wordLimit", "emoji", "login", "highlighter", "texRenderer", "imageUploader", "search", "copyright", "recaptchaV3Key", "turnstileKey", "reaction"], setup(n2, { expose: o2 }) {
    o2();
    const t2 = n2, e2 = { latest: "insertedAt_desc", oldest: "insertedAt_asc", hottest: "like_desc" }, i = Object.keys(e2), s2 = _e(), r2 = ft(), u2 = ref("loading"), l = ref(0), a = ref(1), g = ref(0), b = computed2(() => gn(t2)), A = ref(b.value.commentSorting), f2 = ref([]), S = ref(null), E = ref(null), _ = computed2(() => hn(b.value.dark)), L = computed2(() => b.value.locale);
    ...
    const x = (y2) => {
      ...
      if (E.value)
        E.value.comment = y2.comment, E.value.orig = y2.orig;
      else if ("rid" in y2) {
